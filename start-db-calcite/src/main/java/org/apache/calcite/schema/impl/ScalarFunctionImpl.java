...
/** 实现scalarfunction：标量函数类似于Map，可以把 0 个、 1 个或多个标量值转换成一个标量值，输入是一行数据中的字段，输出则是唯一的值。
...
public class ScalarFunctionImpl extends ReflectiveFunctionBase
    ...
    /** 返回一个map<method名字,根据method构造的scalar函数或table函数>
    ...
    /** 构建一个标量函数基于一个类（类中提供有求值方法，找到该方法后调用下面重载的create）
